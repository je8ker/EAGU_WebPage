<template>
  <v-card class="pa-4 my-4">
    <v-card-title>Edit About Titles</v-card-title>
    <v-card-text>
      <v-form @submit.prevent="saveAbout">
        ...
        <div class="d-flex align-center">
          <v-btn class="justify-start ma-2" text color="green" @click="addTitle">
            <v-icon left>mdi-plus</v-icon>
            Add Title
          </v-btn>
          <v-btn class="justify-end" type="submit" color="primary">
            Save Changes
          </v-btn>
        </div>
      </v-form>
...
const removeTitle = (index) => {
  if (aboutTitles.value.length > 1) {
    if (confirm('이 제목을 삭제하시겠습니까?'))
      aboutTitles.value.splice(index, 1);
  } else {
    alert('제목이 최소 1개는 있어야합니다.');
  }
};

// 수정된 제목 배열을 서버로 전송하는 함수
const saveAbout = () => {
  axios.put('/api/activities', {titles: aboutTitles.value})
    .then(() => {
